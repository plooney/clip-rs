...
pub struct PolyNode {
    ...
    pub tree: ::std::sync::Arc<::std::sync::Mutex<PolyTree>>,
...
impl PolyNode {
    ...
    pub(crate) fn add_child(&mut self, mut child: Self) {
        ...
        child.index = cnt;
        let glob_cnt = tree_lock.nodes.len();
        child.glob_index = PolyNodeIndex { node_idx: glob_cnt };
        tree_lock.nodes.push(child);
    }
    ...
    pub(crate) fn get_next_sibling_up(&self) -> Option<PolyNodeIndex> {
        match self.parent {
            ...
            Some(parent) => {
                ...
                let parent_node = &tree_lock.nodes[parent.node_idx];
        ...
        loop {
            match node_idx {
                Some(idx) => { 
                    ...
                    node_idx = self.tree.lock().unwrap().nodes[idx.node_idx].parent;
                }, 
                None => { break; }
            }
        }
